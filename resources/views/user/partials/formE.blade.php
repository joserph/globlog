...
<div class="form-group row hide" id="farms_input">
    ...
    <div class="col-sm-9">
        <select class="form-control id_farm" name="id_farm" id="id_farm">
            ...
            <option value="{{ $itemFarm->id }}" {{ $itemFarm->id == $user->id_farm ? 'selected' : '' }}>{{ $itemFarm->name }} {{ $itemFarm->tradename }}</option>
            @endforeach
        </select>
    </div>
</div>
<div class="form-group row hide" id="clients_input">
    ...
    <div class="col-sm-9">
        <select class="form-control id_client" name="id_client" id="id_client">
            ...
            <option value="{{ $itemClient->id }}" {{ $itemClient->id == $user->id_client ? 'selected' : '' }}>{{ str_replace('SAG-', '', $itemClient->name) }}</option>
            @endforeach
        </select>
    </div>
</div> 
